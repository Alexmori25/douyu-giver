...
def get_glow():
    ...
    try:
        ...
        logger.info("------背包检查结束------")
    return glow_res.json().get('data', {}).get('list', [])
...
def glow_donate(num=1, room_id=12306, prop_id=268):
    """
    :param num: 向该房间赠送荧光棒的数量
    :param room_id: 房间号
    :param prop_id: 礼物ID, 默认为荧光棒(268)
    """
    donate_url = "/japi/prop/donate/mainsite/v1"
    DATA = "propId=%s&propCount=%s&roomId=%s&bizExt={\"yzxq\":{}}" % (prop_id, num, room_id)
    # 背包中含有道具才会进行赠送，否则会报错
    if Bags:
        ...
        try:
            ...
            logger.info("向房间号%s赠送荧光棒%s个成功,当前剩余%s个" % (room_id, num, now_left))
                ...
                logger.info("向房间号%s赠送礼物(ID:%s)%s个成功" % (room_id, prop_id, num))
        except AssertionError:
            if donate_res.json()['msg'] == "用户没有足够的道具":
                if prop_id == 268:
                logger.warning("向房间号%s赠送荧光棒失败,当前背包中荧光棒数量为:%s,而设定捐赠数量为%s" % (room_id, Own, num))
                else:
                     logger.warning("向房间号%s赠送礼物(ID:%s)失败, 数量不足" % (room_id, prop_id))
            else:
