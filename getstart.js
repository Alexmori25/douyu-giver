              ...
              onload: async (response) => {
                if (response.status === 200 && response.response?.error === 0 && Array.isArray(response.response.data?.redBagList)) {
                  ...
                  const checkPromises = candidateRids.map(async (r) => {
                      const pass = await this.checkRoomRedBag(r);
                      return pass ? r : null;
                  });
          ...
          GM_xmlhttpRequest({
              ...
              onload: (res) => {
                  try {
                      ...
                      // 2. 找到倒计时(waitSec)最短的那个，即“当前最新能领的”
                      const targetRedBag = activeList.reduce((prev, curr) => {
                          return (prev.waitSec < curr.waitSec) ? prev : curr;
                      });

                      // 3. 找到该红包中 ptype === 9 (星光币) 的奖品
                      const prizeList = targetRedBag.prizeList || [];
                      const coinPrize = prizeList.find(p => p.ptype === 9);
                      
                      // 4. 获取星光币数量，如果没有则为 0
                      const coinNum = coinPrize ? (parseInt(coinPrize.num) || 0) : 0;

                      if (coinNum >= SETTINGS.MIN_RED_ENVELOPE_NUM) {
                          resolve(true);
                      } else {
                          resolve(false);
                      }
...
async init() {
    ...
    async findAndExecuteNextTask(roomId) {
      ...
      // [新增] 二次确认：检查当前红包价值是否满足阈值 (进房间后复查)
      if (SETTINGS.MIN_RED_ENVELOPE_NUM > 0) {
          // 稍微延迟一下，避免刚进房间API还没准备好
          await Utils.sleep(500); 
          const isValuable = await DouyuAPI.checkRoomRedBag(roomId);
          if (!isValuable) {
              Utils.log(`[价值检查] 当前房间红包价值不足阈值 (${SETTINGS.MIN_RED_ENVELOPE_NUM})，跳过并切换...`);
